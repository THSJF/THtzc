...
public class Rebound implements Cloneable {
	...
	public Rebound(float xs, float ys, int pc) {
		...
		this.y = ys;
		this.parentcolor = pc;
		this.begin = Layer.LayerArray.get(parentid).begin;
		this.life = Layer.LayerArray.get(parentid).end - Layer.LayerArray.get(parentid).begin + 1;
		this.longs = 100;
		this.time = 1;
		this.angle = 0.0f;
	}
	...
	@Override
	protected Rebound clone() throws CloneNotSupportedException {
		return (Rebound)super.clone();
	}

	public Rebound Clone() {
		try {
			Rebound rb=clone();
			rb.speedds = speedds.cpy();
			rb.aspeedds = aspeedds.cpy();
			rb.rand = rand.Clone();
			rb.copys = copys.Clone();
			rb.Parentevents = new ArrayList<>();
			for (Event et:Parentevents) {
				rb.Parentevents.add(et.Clone());
			}
			return rb;
